// LICENCE https://github.com/adaptlearning/adapt_authoring/blob/master/LICENSE
define(function (require) {
  const OriginView = require('core/views/originView');
  const Origin = require('core/origin');

  const TemplateItemView = OriginView.extend(
    {
      tagName: 'div',

      className: function () {
        return 'template-management-list-item id-' + this.model.get('_id');
      },

      events: {
        click: 'onTemplateClicked',
      },

      preRender: function () {
        this.listenTo(
          Origin,
          'templateManagement:modal:selectItem',
          this.selectItem
        );
        this.listenTo(
          Origin,
          'templateManagement:assetViews:remove',
          this.remove
        );
        this.listenTo(this, 'remove', this.remove);
        this.listenTo(this.model, 'destroy', this.remove);
        this.listenTo(this.model, 'change:_isDeleted', this.onReRender);
      },

      onReRender: function () {
        this.render();
        _.defer(
          _.bind(function () {
            this.onInview();
          }, this)
        );
      },

      postRender: function () {
        if (this.model.get('_isSelected')) {
          this.$el.addClass('selected');
          Origin.trigger(
            'templateManagement:templateItemView:preview',
            this.model
          );
        }
      },

      onTemplateClicked: function () {
        $('.asset-management-list-item').removeClass('selected');
        this.$el.addClass('selected');
        this.model.set('_isSelected', true);
        Origin.trigger(
          'templateManagement:templateItemView:preview',
          this.model
        );
      },

      onInview: function () {
        // Once this asset is inview - change the data-style attribute to the
        // actual style attribute
        const $previewImage = this.$('.asset-management-list-item-image');
        $previewImage.attr('style', $previewImage.attr('data-style'));
        // Remove inview as it's not needed anymore
        this.$el.off('inview');
      },

      selectItem: function (modelId) {
        if (modelId === this.model.get('_id')) {
          this.onTemplateClicked();
        }
      },
    },
    {
      template: 'templateManagementListItem',
    }
  );

  return TemplateItemView;
});
