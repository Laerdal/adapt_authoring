...
describe('applyContentOnlyImport', () => {
  it('patches matched topic/section/group titles and component content by position, never creating/deleting', async () => {
    ...
    // Text component bodies are real multi-paragraph HTML now (each source
    // paragraph keeps its own <p>, formatting preserved), not a bare string —
    // see storyboardGeneration.ts's parseDocToTree.
