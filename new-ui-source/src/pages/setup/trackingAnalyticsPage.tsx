  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className={`w-full px-3 py-2 text-sm rounded-lg border bg-white text-[#111827] focus:outline-none focus:ring-2 focus:border-transparent transition-colors ${
          error ? "border-[#ef4444] focus:ring-[#ef4444]" : "border-[#e5e7eb] focus:ring-[#2d6fa8]"
        }`}
      />
...
function DeploymentModal({
  ...
  const validateDeploymentUrl = (value: string) => {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setDeploymentURL(nextValue);
                if (deploymentUrlError) setDeploymentUrlError(validateDeploymentUrl(nextValue));
              }}
              placeholder="https://..."
              aria-invalid={!!deploymentUrlError}
              className={`w-full px-3 py-2 text-sm rounded-lg border bg-white text-[#111827] focus:outline-none focus:ring-2 focus:border-transparent ${
                deploymentUrlError ? "border-[#ef4444] focus:ring-[#ef4444]" : "border-[#e5e7eb] focus:ring-[#2d6fa8]"
              }`}
            />
            {deploymentUrlError && <span className="text-xs text-[#dc2626]">{deploymentUrlError}</span>}
          </div>
          {type === "standard" && (
            <div className="flex flex-col gap-1.5">
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <button
            type="button"
            onClick={() => {
              const error = validateDeploymentUrl(deploymentURL);
              if (error) {
                setDeploymentUrlError(error);
                return;
              }

              if (type === "standard") {
                onConfirm({ deploymentURL: deploymentURL.trim(), region, environment });
              } else {
                onConfirm({ deploymentURL: deploymentURL.trim(), environment });
              }
            }}
...
export function TrackingAnalyticsPage({
  ...
  const validationErrors = useMemo(() => {
    const uesStandardInvalid = analyticsPlugin === "ues" && ues.standardItems.some((item) => !validateUrl(item.deploymentURL));
    const uesEclInvalid = analyticsPlugin === "ues" && ues.eclItems.some((item) => !validateUrl(item.deploymentURL));

    return {
      uesStandardItems: uesStandardInvalid ? "Each standard entry needs a valid deployment URL." : "",
      uesEclItems: uesEclInvalid ? "Each ECL entry needs a valid deployment URL." : "",
      googleTrackingId: analyticsPlugin === "google" && !google.trackingId.trim() ? "Measurement ID is required." : "",
      hotjarSiteId: analyticsPlugin === "hotjar" && !hotjar.siteId.trim() ? "Site ID is required." : "",
    };
  }, [analyticsPlugin, google.trackingId, hotjar.siteId, ues.eclItems, ues.standardItems]);
          ...
          {analyticsPlugin === "ues" && (
            <div className="mt-4 flex flex-col gap-4">
              ...
              <DeploymentListField
                ...
                error={hasAttemptedSave ? validationErrors.uesStandardItems : ""}
              />
              <DeploymentListField
                ...
                error={hasAttemptedSave ? validationErrors.uesEclItems : ""}
              />
            </div>
          )}
          ...
          {analyticsPlugin === "google" && (
            <div className="mt-4 flex flex-col gap-3">
              ...
              <TextField
                ...
                error={hasAttemptedSave ? validationErrors.googleTrackingId : undefined}
              />
            </div>
          )}
          ...
          {analyticsPlugin === "hotjar" && (
            <div className="mt-4 flex flex-col gap-3">
              ...
              <TextField
                ...
                placeholder="1234567"
                error={hasAttemptedSave ? validationErrors.hotjarSiteId : undefined}
              />
            </div>
          )}
        </AccordionCard>
