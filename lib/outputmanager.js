...
OutputPlugin.prototype.getCourseJSON = function(tenantId, courseId, next) {
  ...
  database.getDatabase(function(err, db) {
    ...
    // OPTIMIZATION: Fetch all content types in PARALLEL instead of sequentially
    var collectionTypes = Object.keys(Constants.CourseCollections);
    var outputJson = {};
    
    async.each(collectionTypes, function(collectionType, callback) {
      if (collectionType === 'config') {
        getConfigJson(courseId, function(err, data) {
          if (err) return callback(err);
          Object.assign(outputJson, data);
          callback();
        });
      } else {
        getContentJson(collectionType, courseId, db, function(err, data) {
          if (err) return callback(err);
          Object.assign(outputJson, data);
          callback();
        });
...
OutputPlugin.prototype.sanitizeCourseJSON = function(mode, json, next) {
  ...
  try {
    ...
    if (!outputJson['course'] || !outputJson['course'].length) {
      return next(new Error('sanitizeCourseJSON: missing course data'));
    }
    if (!outputJson['config'] || !outputJson['config'].length) {
      return next(new Error('sanitizeCourseJSON: missing config data'));
    }
...
OutputPlugin.prototype.addPluginWithDependencies = function (includedPlugins, source_root, plugin_folder, plugin_name, next) {
    ...
    fs.readFile(bower_file, function(err, bower) {
        ...
        try {
          ...
          logger.log('error', 'Failed to parse bower.json', e);
          return next(e);
        }
    ...
    function(seriesCallback) {
      ...
      async.each(Object.keys(props), function(prop, innerCallback) {
        ...
        if (savedSettings.hasOwnProperty(prop) && themeProperty.default !== savedSettings[prop]) {
          ...
          if ((inputType && typeof inputType === 'object' && inputType.media === 'image') || inputType === 'Asset:image') {
    ...
    function(seriesCallback) {
      ...
      database.getDatabase(function(err, db) {
        ...
        db.retrieve('courseasset', {_courseId: courseId, _contentType: 'theme'}, function(err, results) {
          ...
          // PERFORMANCE: Batch retrieve all assets at once instead of N+1 pattern
          ...
          assetmanager.retrieveAsset({ _id: { $in: assetIds } }, function(error, allAssets) {
            ...
            // Process each asset with controlled parallelism (5 concurrent file copies)
        ...
        db.retrieve('courseasset', {_courseId: courseId, _contentType: {$ne: 'theme'}}, {operators: {distinct: '_assetId'}}, function (err, results) {
          ...
          if (results) {
            ...
            assetmanager.retrieveAsset({ _id: {$in: results} }, function (error, assets) {
              ...
              // OPTIMIZATION: Build all asset path replacements FIRST, then apply once
              let assetReplacements = {};
              let lang = jsonObject['config']._defaultLanguage;
              filteredAssets.forEach(function(asset) {
                let replaceRegex = new RegExp("course/assets/" + asset.filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi');
                let newAssetPath = "course/" + lang + "/assets/" + encodeURIComponent(asset.filename);
                assetReplacements[asset.filename] = { regex: replaceRegex, newPath: newAssetPath };
                assetsJson[asset.filename] = { 'title': asset.title, 'description': asset.description, 'tags': asset.tags };
              });

              // Apply ALL replacements in a single JSON stringify/parse cycle
              Object.keys(Constants.CourseCollections).forEach(function(key) {
                let jsonStr = JSON.stringify(jsonObject[key]);
                filteredAssets.forEach(function(asset) {
                  jsonStr = jsonStr.replace(assetReplacements[asset.filename].regex, assetReplacements[asset.filename].newPath);
                });
                try {
                  jsonObject[key] = JSON.parse(jsonStr);
                } catch (e) {
                  logger.log('error', 'Failed to parse JSON after asset replacement for ' + key, e);
                }
              });

              // OPTIMIZATION: Copy assets in parallel (limit 10 concurrent copies)
