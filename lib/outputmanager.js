    ...
    function(seriesCallback) {
      ...
      database.getDatabase(function(err, db) {
        ...
        db.retrieve('courseasset', {_courseId: courseId, _contentType: 'theme'}, function(err, results) {
          ...
          // ADAPT-3614: Process theme assets concurrently (was eachSeries).
          // processedAssets deduplicates any repeated entries in results —
          // worst case the same file is copied twice, which is idempotent.
          async.eachLimit(results, 3, function(result, callback) {
