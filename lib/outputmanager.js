...
OutputPlugin.prototype.getCourseJSON = function(tenantId, courseId, next) {
  ...
  database.getDatabase(function(err, db) {
    ...
    // ADAPT-3614: fetch all course collections in parallel instead of sequentially.
    // Each query is independent; running them concurrently reduces total DB wait time
    // from (N × RTT) to ~1 × RTT, which is material when DB is on a remote host.
    var tasks = Object.keys(Constants.CourseCollections).reduce(function(acc, collectionType) {
      acc[collectionType] = function(callback) {
    ...
    async.parallel(tasks, function(err, results) {
      ...
      // merge { course: {course:[...]}, config: {config:[...]}, ... } → flat outputJson
      var outputJson = Object.keys(results).reduce(function(acc, key) {
        return Object.assign(acc, results[key]);
      }, {});
...
/**
 * Lightweight variant of getCourseJSON that returns ONLY the assembled config
 * (with the derived _enabledComponents/_enabledExtensions), skipping the fetch and
 * assembly of the whole content tree (contentObjects/articles/blocks/components).
 * Callers that only need the course's theme/menu/plugin fingerprint use this to
 * avoid pulling the entire course over the wire on every request.
 */
OutputPlugin.prototype.getCourseConfigJSON = function(courseId, next) {
  getConfigJson(courseId, next);
};
  ...
  origin().contentmanager.getContentPlugin('config', function (err, contentPlugin) {
    ...
    contentPlugin.retrieve({ _courseId: courseId }, {}, function(err, config) {
      ...
      database.getDatabase(function(err, db) {
        ...
        // PERF: fetch component list and component types in parallel (was sequential)
        async.parallel({
          components: function(cb) {
            db.retrieve('component', {_courseId: courseId}, {operators: {distinct: '_component'}}, cb);
          },
          componentTypes: function(cb) {
            db.retrieve('componenttype', {}, { fields: 'name component' }, cb);
          }
        }, function(err, results) {
          if (err) return next(err);

          async.map(results.components, function(component, cb) {
            var componentType = _.findWhere(results.componentTypes, {component: component});
            return cb(null, { name: componentType ? componentType.name : component });
...
OutputPlugin.prototype.generateIncludesForConfig = function(config, callback) {
  ...
  // Use eachLimit for parallel plugin processing (I/O bound)
  async.eachLimit(includedPlugins, 5, function(plugin, next) {
    if (plugin.length < 2) {
...
/**
 * Computes a SHA-256 fingerprint of the full course JSON returned by getCourseJSON.
 * Used by the preview cache gate: if the fingerprint matches the stored .build-hash
 * the pipeline skips all file-writing steps and serves the existing build.
 *
 * Deterministic because getCourseJSON uses retainKeyOrder:true and sorts by _sortOrder.
 */
OutputPlugin.prototype.computeFingerprint = function(outputJson) {
  return crypto.createHash('sha256').update(JSON.stringify(outputJson)).digest('hex');
};

OutputPlugin.prototype.ensureVanillaThemeSemanticCompatibility = function(destinationFolder, next) {
  var themeColorsFile = path.join(destinationFolder, Constants.Folders.Less, '_defaults', '_colors.less');
  var overrideFile = path.join(__dirname, '..', 'plugins', 'output', 'adapt', 'theme-overrides', 'vanilla-semantic-compat.less');
  var compatibilityMarker = '// Adapt Authoring: vanilla semantic compatibility injected';

  fs.readFile(themeColorsFile, 'utf8', function(err, currentContent) {
    if (err) {
      return next(err);
    }

    fs.readFile(overrideFile, 'utf8', function(err, overrideContent) {
      if (err) {
        return next(err);
      }

      if (currentContent.indexOf(compatibilityMarker) !== -1) {
        return next(null);
      }

      var requiredVariables = [];
      var match = null;
      var variablePattern = /@([A-Za-z0-9-]+)\s*:/g;

      while ((match = variablePattern.exec(overrideContent)) !== null) {
        requiredVariables.push(match[1]);
      }

      var hasCompleteContract = requiredVariables.every(function(variableName) {
        return currentContent.indexOf('@' + variableName + ':') !== -1;
      });

      if (hasCompleteContract) {
        return fs.outputFile(themeColorsFile, currentContent + '\n' + compatibilityMarker + '\n', 'utf8', function(err) {
          if (err) {
            return next(err);
          }
          return next(null);
      ...
      fs.outputFile(themeColorsFile, mergedContent, 'utf8', function(err) {
        if (err) {
          return next(err);
        }
        return next(null);
      ...
      fs.remove(destinationFolder, function(err) {
        ...
        fs.copy(themeFolder, destinationFolder, function (err) {
          ...
          self.writeThemeVariables(courseId, theme, themeVariables, destinationFolder, function(error) {
            ...
            if (theme.name === 'adapt-contrib-vanilla') {
              return self.ensureVanillaThemeSemanticCompatibility(destinationFolder, function(err) {
                if (err) {
                  return next(err);
                }
      ...
      async.eachSeries(Object.keys(themeVariables), function(key, innerCallback) {
        ...
        if (!ignoreKeys.includes(key)) {
          if (typeof themeVariables[key] === 'object') {
            ...
            async.eachSeries(Object.keys(themeVariables[key]), function(childKey, secondInnerCallback) {
    ...
    function(seriesCallback) {
      ...
      database.getDatabase(function(err, db) {
        ...
        db.retrieve('courseasset', {_courseId: courseId, _contentType: 'theme'}, function(err, results) {
          ...
          // Batch retrieve all theme assets at once instead of N+1 pattern (ADAPT-3614)
          var assetIds = results.map(function(r) { return r._assetId; });
          assetmanager.retrieveAsset({ _id: { $in: assetIds } }, function(error, allAssets) {
            if (error) {
              return seriesCallback(error);
            }

            // Build lookup map for O(1) access per result
            var assetMap = {};
            allAssets.forEach(function(a) {
              assetMap[a._id.toString()] = a;
            });

            // Process each asset with bounded concurrency (limit raised to 10 for EFS)
            async.eachLimit(results, 10, function(result, callback) {
              var asset = assetMap[result._assetId.toString()];
              if (!asset) {
                logger.log('warn', 'Asset not found for id: ' + result._assetId);
                return callback();
              }
          ...
          if (results) {
            ...
            function collectAssetFilenames(obj, filenames) {
              ...
              if (Array.isArray(obj)) {
                ...
                // Match any file extension (generic)
                let match = obj.match(/([^\/]+\.[a-zA-Z0-9]+)$/i);
                if (match) {
            ...
            assetmanager.retrieveAsset({ _id: {$in: results} }, function (error, assets) {
              ...
              // Phase 1: Apply all asset path replacements synchronously.
              ...
              // Phase 2: Copy files with bounded concurrency (was eachSeries — ADAPT-3614; limit raised to 10).
              async.eachLimit(filteredAssets, 10, function(asset, callback) {
...
OutputManager.prototype.setupRoutes = function () {
  ...
  rest.get('/output/atpluginusage', function (req, res, next) {
    ...
    // Respond immediately — updateAuthoringPluginUsage is fire-and-forget (no callback).
    // Previously the route passed a callback that was never invoked, causing nginx to
    // wait 60 seconds and return 504 Gateway Timeout on every preflight validate call.
    res.json({ success: true });
    that.updateAuthoringPluginUsage(pluginType, feature);
  });
};
