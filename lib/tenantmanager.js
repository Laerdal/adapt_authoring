...
// Short-lived cache for _id-based tenant lookups — avoids a DB round-trip on
// every authenticated request. 30s TTL is safe: tenant config rarely changes.
var _tenantCache = Object.create(null);
var TENANT_CACHE_TTL_MS = 30000;
...
exports = module.exports = {
  ...
  retrieveTenant: function (search, options, callback) {
    ...
    // Cache only single _id lookups — that is the hot path hit on every request.
    var cacheKey = (search && search._id && !options.populate) ? search._id.toString() : null;
    if (cacheKey) {
      var cached = _tenantCache[cacheKey];
      if (cached && (Date.now() - cached.ts) < TENANT_CACHE_TTL_MS) {
        return callback(null, cached.tenant);
      }
    }
    ...
    database.getDatabase(function(err, db) {
      ...
      db.retrieve('tenant', search, options, function (error, results) {
        ...
        if (results && results.length === 1) {
          ...
          if (cacheKey) {
            var plainTenant = JSON.parse(JSON.stringify(tenant));
            // Restore Date objects lost by JSON round-trip — isStale() calls .getTime() on updatedAt
            if (plainTenant.updatedAt) plainTenant.updatedAt = new Date(plainTenant.updatedAt);
            if (plainTenant.createdAt) plainTenant.createdAt = new Date(plainTenant.createdAt);
            _tenantCache[cacheKey] = { tenant: plainTenant, ts: Date.now() };
            return callback(null, plainTenant);
          }
  ...
  invalidateTenantCache: function (tenantId) {
    if (tenantId) delete _tenantCache[tenantId.toString()];
  },
      ...
      self.retrieveTenant(search, function (error, tenantRec) {
        ...
        if (tenantRec) {
          ...
          db.update('tenant', search, update, function(error) {
            ...
            // Evict stale cache entry before re-fetching
            if (tenantRec && tenantRec._id) {
              self.invalidateTenantCache(tenantRec._id);
            }
  ...
  destroyTenant: function (tenantId, callback) {
    ...
    database.getDatabase(function(err, db){
      ...
      self.retrieveTenant({ _id: tenantId }, function (error, tenantRec) {
        ...
        self.invalidateTenantCache(tenantId);
