      ...
      moduleLoaded : function(modloaded){
        ...
        if(modloaded === WAITFOR){
          ...
          rest.post('/login', function (req, res, next) {
            // Store session start time in the session
            if (req.body && req.body.sessionStartTime && req.session) {
              req.session.sessionStartTime = req.body.sessionStartTime;
            }
            ...
            instance.getAuthPlugin(configuration.getConfig('auth'), function (error, authPlugin) {
              ...
              this.sessionStartTime = req.body.sessionStartTime;
            });
          });

          rest.post('/logout', function (req, res, next) {
            var sessionStartTime = (req.body && req.body.sessionStartTime) || (req.session && req.session.sessionStartTime);
            ...
            if (req.body && req.body.email) {
              usermanager.retrieveUser({ email: req.body.email }, function(err, user) {
                if (err || !user) {
                  res.statusCode = 500;
                  return res.json({ error: 'User not found' });
                }
                // Ensure sessions is an array
                var sessions = Array.isArray(user.sessions) ? user.sessions.slice() : [];
                var sessionEndTime = req.body.sessionEndTime || new Date();
                var duration = 0;
                
                if (sessionStartTime && sessionEndTime) {
                  duration = (new Date(sessionEndTime) - new Date(sessionStartTime)) / 1000; // in seconds
                }
                ...
                usermanager.updateUser({ email: req.body.email }, updateData, function(err) {
                  if (err) {
                    res.statusCode = 500;
                    return res.json(err);
                  }
          ...
          rest.get('/authcheck', function (req, res, next) {
            var passport = req.session && req.session.passport;
