...
Origin.prototype.preload = function () {
  ...
  self.once('moduleFailed', function (modName, err) {
    logger.log('error', 'preload error: ' + modName + ' - ' + (err ? err.toString() : 'unknown error'));
    process.exit(1);
  });
...
async function runMigrations() {
  try {
    if (!app || !app.db || !app.db.conn || !app.db.conn.db) {
      logger.log('warn', 'Database not available, skipping migrations');
      return;
    }
...
function checkForUpdates(callback) { 
  installHelpers.getInstalledVersions(function(error, installedData) {
    if (error || !installedData) {
      logger.log('warn', 'Could not retrieve installed versions');
      return callback();
    }
