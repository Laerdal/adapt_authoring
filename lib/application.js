  ...
  */
  ...
  require('./database').getDatabase(function (err, db) {
    ...
    server.use(session({
      ...
      // resave stays true: connect-mongodb-session has no touch(), so this is what
      // keeps an active user's TTL fresh (resave:false would log them out at maxAge).
      resave: true,
      cookie: {
        signed: true,
        secureProxy: app.configuration.getConfig('isProduction')
      },
      // Don't write a session row for every anonymous/static request — that write
      // flood on connect-mongodb-session's old driver is what times out under load
      // and drops req.user. Login writes to the session, so authenticated users are
      // saved normally.
