...
function _handleMasterDB(type, next) {
  ...
  if (db) { // db instance already exists
    if(db.readyState === ReadyStates.READY) { // ...and is connected
      return next(null, db);
    }
...
function _handleSlaveDB(type, tenantId, next) {
  ...
  app.tenantmanager.retrieveTenant({ _id: tenantId }, function (err, tenant) {
    ...
    if(!tenant) {
      return next(new Error('Tenant not found for id: ' + tenantId));
    }
    const dbInstance = _databases[type][tenant._id];
    ...
    createDatabaseInstance(type, tenant.database, next);
  });
}
...
function preloadHandle(app, instance) {
  return {
    ...
    moduleLoaded: function(modloaded) {
      if (modloaded !== WAITFOR) { // not the module that loaded this module's requirement
        return;
      }
