...
// Short-lived cache so multiple permission checks within the same request
// don't each hit MongoDB. Keyed by userId string; entries expire after 5s.
var _policyCache = Object.create(null);
var _policyInflight = Object.create(null); // coalesce concurrent misses per user
var POLICY_CACHE_TTL_MS = 5000;
...
exports = module.exports = {
  ...
  addStatement: function (policy, actions, resource, effect, condition, callback) {
    database.getDatabase(function(err, db){
        ...
        db.update('policy', { _id: policy._id }, { 'statement': statement }, function(err) {
          if (!err && policy.user) this.invalidatePolicyCache(policy.user);
          callback(err);
        }.bind(this));
  ...
  getUserPolicyStatements: function (userId, callback) {
    ...
    if (cached && (Date.now() - cached.ts) < POLICY_CACHE_TTL_MS) {
      return callback(null, cached.statements);
    }

    // Coalesce concurrent cache-misses for the same user. The course-listing
    // permission loop fires one of these per course, all at once — for a user
    // with 1000s of courses that stampeded the DB before the cache could
    // populate (40s+ to list). Share a single in-flight fetch; the rest wait.
    if (_policyInflight[cacheKey]) {
      _policyInflight[cacheKey].push(callback);
      return;
    }
    _policyInflight[cacheKey] = [callback];

    function done(err, statements) {
      if (!err) {
        _policyCache[cacheKey] = { statements: statements, ts: Date.now() };
      }
      var waiters = _policyInflight[cacheKey] || [];
      delete _policyInflight[cacheKey];
      waiters.forEach(function (cb) { cb(err, statements); });
    }
    ...
    database.getDatabase(function(err, db) {
      db.retrieve('user', { _id: userId }, { fields: 'roles _tenantId', populate: ['roles'] }, function(err, userDocs) {
        ...
        if (1 !== userDocs.length) {
          return done(new Error('Failed to find unique user record'));
        }
        ...
        db.retrieve('policy', { user: userId }, function(error, policyDocs) {
          ...
          if (!policyDocs || !policyDocs.length) {
            return done(null, statements);
          }
          // add user-specific policy statements
          policyDocs.forEach(p => statements.push(...p.statement));
          done(null, statements);
        });
      });
    }, configuration.getConfig('dbName'));
  },

  // Invalidate the policy cache for a user when their roles or policies change.
  invalidatePolicyCache: function (userId) {
    delete _policyCache[userId.toString()];
  },
  ...
  clearPolicies: function (userId, callback) {
    this.invalidatePolicyCache(userId);
    database.getDatabase(function(err, db){
  ...
  policyChecker: function () {
    return (req, res, next) => {
      ...
      const user = app.usermanager.getCurrentUser() || (req.session && req.session.passport && req.session.passport.user ? JSON.parse(JSON.stringify(req.session.passport.user)) : null);
