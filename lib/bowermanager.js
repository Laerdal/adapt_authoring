      ...
      bower.commands.search(pluginName, bowerOptions)
        ...
        bower.commands.install([packageName], { save: true }, bowerOptions)
          ...
          .on('end', function (packageInfo) {
            if (!packageInfo || !packageInfo[pluginName] || !packageInfo[pluginName].pkgMeta) {
              logger.log('error', 'No package info found for ' + pluginName);
              return callback('No package info found for ' + pluginName);
            }
            ...
            async.doUntil(function iterator(cb) {
              ...
              app.contentmanager.getContentPlugin(pluginType, (error, plugin) => {
                ...
                app.db.retrieve(plugin.getPluginType(), {
                  ...
                  var installedPlugin = (results && results.length) ? results[0] : null;
