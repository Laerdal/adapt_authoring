...
ContentPlugin.prototype.create = function (data, next) {
  ...
  database.getDatabase(function (error, db) {
    ...
    data._tenantId = tenantId;

    // Check if the user has permission on this course
    self.hasPermission('create', user._id, tenantId, data, function (err, isAllowed) {
      if (!isAllowed) {
        return next(new ContentPermissionError());
      }
...
ContentPlugin.prototype.update = function (search, delta, next) {
  ...
  var tenantId = user.tenant && user.tenant._id;
  self.hasPermission('update', user._id, tenantId, delta, function (err, isAllowed) {
    if (!isAllowed) {
      return next(new ContentPermissionError());
    }
  ...
  rest.post('/content/clipboard/copy', function(req, res, next) {
    ...
    database.getDatabase(function (error, db) {
      ...
      async.series([
        ...
        // Process blocks - OPTIMIZED: batch query using $in instead of N+1 pattern
        function(callback) {
          if(articles.length === 0 || isMenu) {
            return callback();
          }
          // Collect all article IDs and fetch blocks in one query
          var articleIds = articles.map(function(article) {
            return article._id;
          });
          db.retrieve('block', { _parentId: { $in: articleIds } }, function(error, results) {
            if (error) {
              return callback(error);
            }
            results.forEach(function(result) {
        ...
        // Process components - OPTIMIZED: batch query using $in instead of N+1 pattern
        function(callback) {
          if (blocks.length === 0 || isMenu) {
            return callback();
          }
          // Collect all block IDs and fetch components in one query
          var blockIds = blocks.map(function(block) {
            return block._id;
          });
          db.retrieve('component', { _parentId: { $in: blockIds } }, function(error, results) {
            if (error) {
              return callback(error);
            }
            results.forEach(function(result) {
