...
var path = require('path'),
    ...
    componentPropertyDefaults = require('./componentPropertyDefaults'),
...
/**
 * Ensure the current content plugin's model is registered on the active database.
 * This recovers from cases where a plugin route is loaded but its schema was not
 * added during startup hook processing.
 *
 * @param {object} db
 * @param {callback} next
 */
ContentPlugin.prototype.ensureModelLoaded = function (db, next) {
  var modelName = this.getModelName && this.getModelName();
  if (!modelName) {
    return next(new ContentTypeError('this.getModelName() must be set!'));
  }

  if (db.getModel && db.getModel(modelName)) {
    return next(null);
  }

  this.onDatabaseCreated(db, function (err) {
    if (err) {
      return next(err);
    }
    ...
    return next(new Error('Failed to load content model: ' + modelName));
  });
};
...
ContentPlugin.prototype.create = function (data, next) {
  ...
  database.getDatabase(function (error, db) {
    ...
    self.hasPermission('create', user._id, user.tenant._id, data, function (err, isAllowed) {
      ...
      self.ensureModelLoaded(db, function (err) {
        if (err) {
          return next(err);
        }
...
ContentManager.prototype.setupRoutes = function () {
  ...
  rest.get('/content/schema', function(req, res, next) {
    ...
    function generateComponentProperties(schema) {
      ...
      componentPropertyDefaults.applyDefaultsToSchema(data.properties, schema.component);
      return data;
    }
  ...
  /**
   * Batch retrieve endpoint - fetches all content items for multiple parent IDs in a single request.
   * This significantly improves performance by reducing N+1 queries to a single query.
   * 
   * POST /api/content/:type/batch
   * Body: { parentIds: ['id1', 'id2', ...], courseId: 'courseId' }
   * 
   * Returns: Array of content items grouped by _parentId
   */
  rest.post('/content/:type/batch', function (req, res, next) {
    var type = req.params.type;
    var parentIds = req.body.parentIds;
    var courseId = req.body.courseId;

    if (!parentIds || !Array.isArray(parentIds) || parentIds.length === 0) {
      res.statusCode = 400;
      return res.json({ success: false, message: 'parentIds array is required' });
    }

    // Limit batch size to prevent abuse
    if (parentIds.length > 500) {
      res.statusCode = 400;
      return res.json({ success: false, message: 'Maximum 500 parent IDs per batch request' });
    }

    var search = {
      _parentId: { $in: parentIds }
    };

    if (courseId) {
      search._courseId = courseId;
    }
    ...
    that.retrieve(type, search, options, function (error, results) {
      ...
      if (type !== 'component') {
        ...
        // For components, append componentTypeDisplayName
        database.getDatabase(function (err, db) {
          if (err) {
            return next(err);
          }
          ...
          db.retrieve('componenttype', {}, function (err, componentTypes) {
            ...
            results.forEach(function(component) {
              ...
              component._doc._componentTypeDisplayName = match ? match.displayName : '';
            });
  ...
  /**
   * Bulk retrieve all content for a page/menu in a single request.
   * Returns all articles, blocks, and components for the given content object.
   * 
   * GET /api/content/page/:id/all
   * 
   * Returns: { articles: [...], blocks: [...], components: [...] }
   */
  rest.get('/content/page/:id/all', function (req, res, next) {
    ...
    async.waterfall([
      // Step 1: Get all articles for this page
      function(callback) {
        that.retrieve('article', { _parentId: pageId }, options, function(error, articles) {
          if (error) return callback(error);
          result.articles = articles;
          callback(null, articles.map(function(a) { return a._id.toString(); }));
        });
      },
      // Step 2: Get all blocks for all articles in one query
      function(articleIds, callback) {
        if (articleIds.length === 0) {
          return callback(null, []);
        }
        that.retrieve('block', { _parentId: { $in: articleIds } }, options, function(error, blocks) {
          if (error) return callback(error);
          result.blocks = blocks;
          callback(null, blocks.map(function(b) { return b._id.toString(); }));
        });
      },
      // Step 3: Get all components for all blocks in one query
      function(blockIds, callback) {
        if (blockIds.length === 0) {
          return callback(null);
        }
        that.retrieve('component', { _parentId: { $in: blockIds } }, options, function(error, components) {
          if (error) return callback(error);
          
          // Append componentTypeDisplayName
          database.getDatabase(function (err, db) {
            if (err) return callback(err);
  ...
  rest.post('/content/clipboard/copy', function(req, res, next) {
    ...
    database.getDatabase(function (error, db) {
      ...
      async.series([
        ...
        // Process blocks — single query for all articles using $in
        function(callback) {
          if(articles.length === 0 || isMenu) {
            return callback();
          }
          var articleIds = articles.map(function(a) { return a._id; });
          db.retrieve('block', { _parentId: { $in: articleIds } }, { operators: { sort: { _sortOrder: 1 } } }, function(error, results) {
            if (error) return callback(error);
            results.forEach(function(r) { blocks.push(r._doc); });
            callback();
          });
        },
        // Process components — single query for all blocks using $in
        function(callback) {
          if (blocks.length === 0 || isMenu) {
            return callback();
          }
          var blockIds = blocks.map(function(b) { return b._id; });
          db.retrieve('component', { _parentId: { $in: blockIds } }, { operators: { sort: { _sortOrder: 1 } } }, function(error, results) {
            if (error) return callback(error);
            results.forEach(function(r) { components.push(r._doc); });
            callback();
          });
        },
