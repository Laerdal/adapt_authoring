...
// AI-Tutor tag ids (general + per-course). These barely ever change (only
// when _ensureAiTutorTags below (re)provisions them), but _resolveAiTutorTags
// was re-querying them via db.retrieve on EVERY asset list request - even
// plain "show me my assets" calls with nothing to do with AI Tutor - adding
// a full extra DB round-trip each time. Confirmed live: this lookup alone
// was most of the gap between a ~260ms templating query and a ~940ms asset
// query against the same DB. Cached the same way lib/permissions.js caches
// its own repeated lookups.
var _tagIdCache = Object.create(null); // title -> { id, at }
var TAG_ID_CACHE_TTL_MS = 60000;

function getCachedTagId (db, title, callback) {
  var hit = _tagIdCache[title];
  if (hit && (Date.now() - hit.at) < TAG_ID_CACHE_TTL_MS) {
    return callback(null, hit.id);
  }
  db.retrieve('tag', { title: title }, function (err, docs) {
    if (err) return callback(err, null);
    var id = (docs && docs.length) ? docs[0]._id : null;
    _tagIdCache[title] = { id: id, at: Date.now() };
    callback(null, id);
  });
}
...
exports = module.exports = {
  ...
  retrieveAsset: function (search, options, next) {
    ...
    database.getDatabase(function (error, db) {
      ...
      __this.hasPermission('delete', user._id, user.tenant._id, '*', function(error, isAllowed) {
        ...
        if(!isAllowed) {
          // Legacy assets may not have _isDeleted set. Treat only explicit true as deleted.
          search = _.extend(search, { _isDeleted: { $ne: true } });
        }
  ...
  _resolveAiTutorTags: function (courseId, callback) {
    database.getDatabase(function (err, db) {
      if (err) return callback(err, null);
      getCachedTagId(db, AITUTOR_TAG, function (e1, general) {
        if (e1) return callback(e1, null);
        if (!courseId) return callback(null, { general: general, course: null });
        getCachedTagId(db, AITUTOR_TAG + '-' + courseId, function (e2, course) {
          if (e2) return callback(e2, null);
          callback(null, { general: general, course: course });
        });
      });
    });
  },
    ...
    async.each(
      ...
      function () {
        ...
        self._resolveAiTutorTags(options.aiTutorCourseId, function (tagErr, tags) {
          ...
          if (Array.isArray(query.$and) && !query.$and.length) {
            delete query.$and;
          }
          runQuery();
        });
