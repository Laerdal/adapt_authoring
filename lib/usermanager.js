...
exports = module.exports = {
  ...
  updateUser: function (search, update, callback) {
    ...
    this.retrieveUser(search, function (error, result) {
      ...
      database.getDatabase(function(err, db) {
        ...
        if (update.email !== (currentUser && currentUser.email)) {
  ...
  setUserActive: function (user, active, callback) {
    ...
    this.retrieveUser(user, function (error, result) {
      if (error) {
        ...
        self.updateUser({ '_id': result._id }, { 'active': active }, callback);
      } else {
  ...
  getCurrentUser: function () {
    try { // parse the session user to give us a plain object without ObjectIds
      ...
      // Use spread operator instead of JSON.parse/stringify for better performance
      return {
        ...user,
        _id: user._id ? user._id.toString() : user._id,
        tenant: user.tenant ? { ...user.tenant, _id: user.tenant._id ? user.tenant._id.toString() : user.tenant._id } : user.tenant
      };
    } catch(e) {
      return false;
    }
  ...
  resetUserPassword: function (user, callback) {
    ...
    this.retrieveUserPasswordReset({token:user.token}, function (err, resetRequest) {
      ...
      if (!resetRequest) {
        return callback(new Error('Password reset request not found'), false);
      }
  ...
  init: function (app) {
    ...
    // Get user details with specific schema format - No authentication required
    rest.get('/userDetails', function (req, res, next) {
      const search = req.query.search || {};
      let andList = [];
      let orList = [];
      // convert searches to regex
      ...
      // Use self reference to avoid 'this' context issues in regular function
      self.retrieveUsers(query, { populate: { 'roles': 'name', '_tenantId': 'name' } }, function(err, users) {
        ...
        // Format users according to the specified schema
    ...
    // Email-based authentication bypass endpoint
    rest.get('/user/email/:emailId', function (req, res, next) {
      // Check if email authentication bypass is enabled
      if (!configuration.getConfig('emailAuthBypassEnabled')) {
        res.statusCode = 404;
        return res.json({ success: false, message: 'Endpoint not found' });
      }

      var emailId = req.params.emailId;
      
      if (!emailId || 'string' !== typeof emailId) {
        res.statusCode = 400;
        return res.json({ success: false, message: 'Valid email ID is required' });
      }

      // Retrieve user by email
      self.retrieveUser({ email: emailId.toLowerCase() }, function (err, user) {
        if (err) {
          res.statusCode = 500;
          return res.json({ success: false, message: 'Database error', error: err });
        }
        ...
        if (user.active === false) {
          res.statusCode = 401;
          return res.json({ success: false, message: 'User account is disabled' });
        }
        ...
        tenantmanager.retrieveTenant({ _id: user._tenantId }, function (err, tenant) {
          if (err) {
            res.statusCode = 500;
            return res.json({ success: false, message: 'Tenant check failed', error: err });
          }

          if (!tenant || tenant._isDeleted) {
            res.statusCode = 401;
            return res.json({ success: false, message: 'Tenant is disabled' });
          }

          // Log access
          self.logAccess(user, function(err) {
            if (err) {
              logger.log('error', 'Failed to log access for user: ' + user.email, err);
            }

            // Serialize user for session
            self.serializeUser(user, function(err, sessionUser) {
              if (err) {
                res.statusCode = 500;
                return res.json({ success: false, message: 'Session creation failed', error: err });
              }

              // Create session manually by setting session data
              req.session.passport = { user: sessionUser };
              req.session.save(function(err) {
                if (err) {
                  res.statusCode = 500;
                  return res.json({ success: false, message: 'Session save failed', error: err });
                }

                logger.log('info', 'Email-based authentication successful for user: ' + user.email);
